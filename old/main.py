bags = """pale cyan bags contain 2 posh black bags, 4 wavy gold bags, 2 vibrant brown bags.
...
pale magenta bags contain 1 clear gold bag, 5 posh fuchsia bags, 2 faded cyan bags.""".splitlines()
...
for rule in bags:
  ...
  if (text[4] == "no"):
    ...
    subrules = {"".join(r.strip().split(" ")[1:3]): r.strip().split(" ")[0] for r in contain.split(",")}

    rules[bag] = subrules

count = 0

def calculate(bag, mult):
  global count

  if rules[bag]:
    for bags in rules[bag]:
      count += int(rules[bag][bags]) * mult
      
      calculate(bags, int(rules[bag][bags]) * mult)

calculate("shinygold", 1)
